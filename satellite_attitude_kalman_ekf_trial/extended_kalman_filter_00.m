...
if isempty(qk)
    ...
    wkm   = w_B_BI_m;                            % Initial Value from measured gyro
    dwk   = [0;0;0];
...
R_B_I  = ATT(qk1);    % Attitude Transformation Matrix of  Quaternion (xyzw) from Inertial to Body
...
for i = 1:length(mflag)
     if( (mflag(i) == 1) && (i <= MaxST) )
            ...
            % Update
            Pk1  = (eye(12) - K*H)*Pk1;          
            res  = e_B_I_m(1:3,i) - qk1(1:3,1); % quaternion
...
wkm1   = w_B_BI_m - biask1 ; %  Estimated Angular Velocity with Noise
...
%% OUTPUT
w_B_BI_f = wk1;
q_B_I_f  = qinvert(qk1,'xyzw');         % Quaternions [w;x;y;z] 
