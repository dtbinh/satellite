...
%% INPUT
R = input(1:3);  % [m] Position Vector   (Inertial Frame)
V = input(4:6);  % [m/s] Velocity Vector (Inertial Frame)
...
%% PERIFOCAL FRAME 
H = cross(R,V);                    % [m^2/s] Angular Momentum Vector Z axis of Perifocal Frame (Inertial Frame)
N = cross([0;0;1],H);              % [] Nodal Vector N of Perifocal Frame pointing to ascending node (Inertial Frame)
...
E = 1/mu*(cross(V,H)-mu*R/r);         % [-] Eccentricity vector pointing to Periapsis (Inertial Frame) (Wertz 6-9)
E = E/norm(E);                        % [-] Unit Eccentricity vector pointing to Periapsis
Y = cross(H,E);                       % [-] Y vector of Perifocal Frame
TA = acos(dot(E,R)/norm(E)/norm(R));  % [rad] True Anomaly - angle between periapsis and satellite
